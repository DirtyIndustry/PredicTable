...
namespace PredicTable.Ajax
{
    ...
    /// gettablequery 的摘要说明
    /// </summary>
    public class gettablequery : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (method == "getdata")
                {
                    ...
                    sql_TableQuery bumen = new sql_TableQuery();
                    rizhi.YBDSIZE = "";
                    ...
                    for (int i = 0; i < bumentable.Rows.Count; i++)
                    {
                        ...
                        sb.Append("{\"id\":\""
                           + bumentable.Rows[i]["YBDID"] + "\",\"wjdx\":\""
                           + bumentable.Rows[i]["YBDSIZE"] + "\",\"sczz\":\""
                           + bumentable.Rows[i]["UPLOADER"] + "\",\"wjm\":\""
                           + bumentable.Rows[i]["YBDNAME"] + "\",\"scsj\":\""
                           + bumentable.Rows[i]["UPLOADDATE"] + "\"},");
                    }
                ...
                else if (method == "getbyid")
                {
                    ...
                    //string zippath = System.Web.HttpContext.Current.Server.MapPath("/download/down.zip");
                    ...
                    //if (ZipCom(fileby.ToArray(), filename.ToArray(), zippath))
                    //{
                    //    context.Response.AddHeader("content-type", "application/x-msdownload");
                    //    //context.Response.AddHeader("Content-Disposition", "attachment;filename=down.zip");
                ...
                else if (method == "showbyid")
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (getwordbybyte(showbyte))
                        {
                            //生成流文件成功
                            //生成html
                          //    ConvertPdfToSwf(zippath,System.Web.HttpContext.Current.Server.MapPath("/download/show.swf"));
                          wordToHtml(zippath);
                        }
                        else
                        {
