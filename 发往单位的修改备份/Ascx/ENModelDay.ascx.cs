...
namespace PredicTable.Ascx
{
    ...
    /// 英文预报-旬、月预报
    /// </summary>
    public partial class ENModelDay : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["BackImage"] != null)
            {
                ...
                //发布时间、发布时效、发布单位
                if (Session["ENEffectTime"] != null && Session["ENPublishTime"] != null && Session["ENFBDW"] != null)
                {
                    reponse +=     "$('#hid_fbdw').val('" + Session["ENFBDW"].ToString() + "');"
                                   + "$('#hid_effectTime').val('" + Session["ENEffectTime"].ToString() + "');"
