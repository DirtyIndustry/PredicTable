                    ...
                    if (dtsw != null && dtsw.Rows.Count > 0)
                    {
                        for (int j = 0; j < dt.Rows.Count; j++)
                        {
                            ...
                            for (int k = 0; k < dtsw.Rows.Count; k++)
                            {
                                ...
                                {
                                    case "刁口":
                                        sb_str.Append("\",\"dl\":\"" + dt.Rows[j]["EFWWDKSEAAREAWAVEHEIGHT"]
                                        + "\",\"ds\":\"" + dtsw.Rows[k]["MEAN_24H"]);
                                        break;
                                    case "黄河口":
                                        sb_str.Append("\",\"hl\":\"" + dt.Rows[j]["EFWWHHKSEAAREAWAVEHEIGHT"]
                                    + "\",\"hs\":\"" + dtsw.Rows[k]["MEAN_24H"]);
                                        break;
                                    case "广利（羊口）":
                                        sb_str.Append("\",\"gl\":\"" + dt.Rows[j]["EFWWGLGSEAAREAWAVEHEIGHT"]
                                   + "\",\"gs\":\"" + dtsw.Rows[k]["MEAN_24H"]);
                                        break;
                                    case "东营":
                                        sb_str.Append("\",\"dyl\":\"" + dt.Rows[j]["EFWWDYGWAVEHEIGHT"]
                                   + "\",\"dys\":\"" + dtsw.Rows[k]["MEAN_24H"]);
                                        break;
                                    case "新户":
                                        sb_str.Append("\",\"xl\":\"" + dt.Rows[j]["EFWWXHWAVEHEIGHT"]
                                   + "\",\"xs\":\"" + dtsw.Rows[k]["MEAN_24H"]);
                                        break;
                                    case "埕口海洋站":
                                        sb_str.Append("\",\"cl\":\"" + dt.Rows[j]["EFWWCKWAVEHEIGHT"]
                                   + "\",\"cs\":\"" + dtsw.Rows[k]["MEAN_24H"]);
                                        break;
                                    default:
                                        break;
                                }
